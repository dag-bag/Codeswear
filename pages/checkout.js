...
export default function Checkout({
  ...
  const initiatePayment = async (e) => {
    ...
    const resp = fetch("http://localhost:3000/api/pretransaction", {
      method: "POST", // or 'PUT'
      headers: {
    ...
    const txnToken = await resp.json;
    console.log(`$paytm response:${txnToken}`);
    var config = {
      root: "",
      flow: "DEFAULT",
      data: {
  ...
  return (
    <div>
      ...
      <Script
        ...
        src={`https://securegw.paytm.in/merchantpgpui/checkoutjs/merchants/ACMcaY34980573593721.js`}
      ></Script>
                  ...
                  {Object.keys(Cart).map((k) => {
                    return (
                      <div className="flex space-x-4" key={k}>
                        <div>
                          <img src={Cart[k].img} alt="image" className="w-60" />
                        </div>
                        <div>
